...
function parseSubjects(input, overallDifficulty) {
  if (Array.isArray(input)) {
    return input
      .map((item) => {
        if (!item) return null;
        if (typeof item === "string") {
          return { name: item.trim(), difficulty: normalizeDifficulty(overallDifficulty) };
        }
        if (typeof item === "object") {
          return {
            name: String(item.name || "").trim(),
            difficulty: normalizeDifficulty(item.difficulty || overallDifficulty)
          };
        }
        return null;
      })
    ...
    return input
      ...
      .map((token) => {
        ...
        if (match) {
          ...
          return { name: match[1].trim(), difficulty: normalizeDifficulty(level) };
        }
        return { name: token, difficulty: normalizeDifficulty(overallDifficulty) };
      });
  }
...
function buildPlan(subjects, examDate, hoursPerDay) {
  const hours = Math.max(1, parseInt(hoursPerDay, 10) || 3);
...
function generatePlanner(req, res) {
  const { subjects, examDate, hoursPerDay, overallDifficulty } = req.body;
...
module.exports = { generatePlanner };
